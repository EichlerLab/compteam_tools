...
def find_contigs(wildcards):
	return gather.split('tmp/{sample}.{scatteritem}-broken.paf', sample=wildcards.sample)
...
rule make_paf:
	...
	shell:
		'''
		minimap2 -c -t {threads} -K {resources.mem}G --eqx --cs {MINIMAP_PARAMS} --secondary=no --eqx -Y {input.ref} {input.fa} > {output.paf}
		'''
...
rule make_sam:
	...
	shell:
		'''
		minimap2 -c -t {threads} -K {resources.mem}G --cs -a {MINIMAP_PARAMS} --secondary=no --eqx -Y {input.ref} {input.fa} | samtools view -S -b /dev/stdin | samtools sort -@ {threads} /dev/stdin > {output.paf}
		'''
